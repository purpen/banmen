...
@interface THNSalesOrderTableViewCell()
...
@implementation THNSalesOrderTableViewCell
...
-(void)setModelAry:(NSArray *)modelAry{
    ...
    if (model.order_count == NULL) {
        ...
        if (_unitFlag) {
            self.topLeftTwoLabel.text = [NSString stringWithFormat:@"订单数：%.2f万", [model.order_count floatValue]/10000];
...
{
    if (_unitFlag) {
        self.topLeftTwoLabel.text = [NSString stringWithFormat:@"订单数：%.2f万", entry.y];
    } else {
        self.topLeftTwoLabel.text = [NSString stringWithFormat:@"订单数：%.2f", entry.y];
    }
